...
def perform_login(login_page, login="", password=""):
    login_input = login_page.find_element(*LOGIN_INPUT)
    password_input = login_page.find_element(*PASSWORD_INPUT)
    login_button = login_page.find_element(*LOGIN_BUTTON)
...
def get_error_message(login_page):
    return WebDriverWait(login_page, 5).until(
...
def test_button_login_enabled(login_page):
    button = login_page.find_element(*LOGIN_BUTTON)
